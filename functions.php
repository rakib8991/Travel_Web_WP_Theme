...
/*------------------------------------*\
    Require Once 
\*------------------------------------*/
if(file_exists(dirname(__FILE__).'/lib/widget.php')){
...
function slider_index(){
    ...
    echo '<div class="owl-carousel owl-theme">'; 
        $slider = new WP_Query($args); while($slider->have_posts()) : $slider->the_post();
    echo    '<div>';
...
add_action('slider','slider_index');
...
if ( function_exists( 'add_theme_support' ) ) {
    ...
    add_image_size( 'blog_post_image_size', '1280', '444', true ); // Blog Post Size Thumbnail.
    add_image_size( 'home_slider', '1500', '500', true ); // Blog Post Size Thumbnail.
...
function html5blank_header_scripts() {
    if ( $GLOBALS['pagenow'] != 'wp-login.php' && ! is_admin() ) {
        if ( HTML5_DEBUG ) {
            ...
            // fontawesome
            wp_register_script( 'fontawesome', 'https://kit.fontawesome.com/3346ef622e.js', array(), '4.3.0' );
            ...
            // Custom scripts
            wp_register_script('html5blankscripts',get_template_directory_uri() . '/js/scripts.js',array(),'1.0.0','true' );

            // Magnific Popup 
            wp_register_script('magnific',get_template_directory_uri() . '/js/jquery.magnific-popup.min.js',array('jquery'),'1.0.0','true' );

             // Olw carousel
             wp_register_script('owl_carousel',get_template_directory_uri() . '/js/owl.carousel.min.js',array('magnific','jquery'),'1.0.0','true' );

            // Shear Tool addthis.com 
            wp_register_script( 'shear_tool_script','//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-6312cf23690a4b48',array(),'2.0.1',);
            ...
            wp_enqueue_script('shear_tool_script');
...
function html5blank_styles() {
    if ( HTML5_DEBUG ) {
        ...
        // googleapis  CSS
        wp_register_style( 'googleapis', "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@700&display=swap", array(), '2.0.1' );

        // googleapisp  Register CSS
        wp_enqueue_style( 'googleapis' );

        // Magnific popup  CSS
        wp_register_style( 'magnific_style', get_template_directory_uri() . '/css/magnific-popup.css', array( 'normalize' ), '2.0.1' );

        // Magnific popup  Register CSS
        wp_enqueue_style( 'magnific_style' );

        // Olw Carousel style
        wp_register_style( 'olw_carousel_style', get_template_directory_uri() . '/css/owl.carousel.min.css', array( 'normalize' ), '2.0.1' );

        // Magnific popup  Register CSS
        wp_enqueue_style( 'olw_carousel_style' );

        // Olw Carousel Theme style
        wp_register_style( 'olw_carousel_theme_style', get_template_directory_uri() . '/css/owl.theme.default.min.css', array( 'normalize' ), '2.0.1' );

        // Olw Carousel Theme style
        wp_enqueue_style( 'olw_carousel_theme_style' );
